...
def searchName(request):
    ...
    f = Friend.objects.filter(friend_name__contains = sName)
...
def delete_friend(request, id):
    ...
    # Friend 객체 중에 파라미터로 넘어온 id와 일치하는 객체 삭제
    Friend.objects.get(id = id).delete()

    # 주소 호출할 때 앞에 아무것도 안붙이면, 현재 디렉토리에서 이동
    # 상위 디렉토리: ../    하위 디렉토리: ./
...
def update_friend(request, id):
    ...
    if request.method == 'GET':
        ...
        # id로 찾은 객체를 폼에서 수정한 값으로 원래 객체 덮어쓰기
